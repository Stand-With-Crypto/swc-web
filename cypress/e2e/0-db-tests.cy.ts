...
describe('ensure basic MySQL database functionalities work', () => {
  it('create a table', function () {
    cy.queryDb('CREATE TABLE person (age int, first_name varchar(255), city varchar(255))')
  })
  ...
  it('update an entry in the table and verify', function () {
    cy.queryDb(`UPDATE person SET city="New York" WHERE first_name="Banana"`).then(
      (result: any) => {
  ...
  it('verify that there is only one row where the city is "San Jose"', function () {
    cy.queryDb(`SELECT COUNT(*) AS "row_count" FROM person WHERE city="San Jose"`).then(
      (result: any) => {
  ...
  it('delete a table and verify', function () {
    cy.queryDb(`DROP TABLE person`).then((result: any) => {
