    ...
    try {
      ...
      if (stateRacesData) {
        ...
        if (
          stateRacesData.some(
            currentRace =>
              +currentRace.totalVotes > 0 ||
              currentRace.candidatesWithVotes.some(currentCandidate => +currentCandidate.votes > 0),
          )
        ) {
          allCompletedRaces.statesWithVotes.push(currentStateKey)
        }
