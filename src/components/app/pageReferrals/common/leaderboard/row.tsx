...
interface LeaderboardRowProps extends VariantProps<typeof rowVariants> {
  count: number
  rank: number
...
export function LeaderboardRow(props: LeaderboardRowProps) {
  const { rank, count, className, variant, locale, children } = props
...
function LeaderboardRowLabel({ children }: { children: React.ReactNode }) {
  return <div className="font-semibold capitalize">{children}</div>
}

LeaderboardRow.Label = LeaderboardRowLabel
