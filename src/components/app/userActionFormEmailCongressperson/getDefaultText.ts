export function getDefaultText(dtsiPersonSlugs: string[]) {
  if (dtsiPersonSlugs.some(slug => HOUSE_LEADERSHIP_SLUGS.includes(slug))) {
    return 'PHASE 1 TEXT TODO'
  }
  if (dtsiPersonSlugs.some(slug => KEY_HOUSE_REP_SLUGS.includes(slug))) {
    return 'PHASE 1.5 TEXT TODO'
  }
  return `The House Financial Services Committee and the House Agriculture Committee in the U.S. House of Representatives passed historic, bipartisan legislation in July. I am asking you to support the bill when it comes to the floor for a full House vote.
