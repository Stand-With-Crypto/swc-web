      ...
      {INDUSTRY_PARTNERS.map(({ title, url, imageSrc }) => {
        return (
          <ExternalLink href={url} key={title}>
            <MemoizedPartnerImage imageSrc={imageSrc} title={title} />
          </ExternalLink>
        )
      })}
    </div>
...
const MemoizedPartnerImage = memo(({ title, imageSrc }: { title: string; imageSrc: string }) => {
  return (
...
MemoizedPartnerImage.displayName = 'MemoizedPartnerImage'
