...
export function AULocationStateSpecific({
  ...
  return (
    <div>
      ...
      {isEmpty(groups.houseOfReps) && isEmpty(groups.senate) ? (
        <PageTitle as="h3" className="mt-20" size="sm">
          There's no key races currently in {stateName}
        </PageTitle>
      ) : (
        <div className="space-y-20">
          {!!groups.houseOfReps.length && (
            <div className="mt-20">
              <DTSIPersonHeroCardSection
                countryCode={countryCode}
                cta={
                  <InternalLink href={urls.locationStateSpecificHouseOfRepsRace(stateCode)}>
                    View Race
                  </InternalLink>
                }
                people={groups.houseOfReps}
                title={<>House of Representatives Race ({stateCode})</>}
              />
            </div>
          )}
