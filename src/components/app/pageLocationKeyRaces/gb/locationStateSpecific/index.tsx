...
export function GBLocationStateSpecific({
  ...
  const stateName = getGBCountryNameFromCode(stateCode)
  ...
  return (
    <div>
      ...
      {isEmpty(groups.houseOfCommons) && isEmpty(groups.houseOfLords) ? (
        ...
        <div className="space-y-20">
          ...
          {groups.houseOfLords.length && (
            <div className="mt-20">
              <DTSIPersonHeroCardSection
                countryCode={countryCode}
                cta={
                  <InternalLink href={urls.locationStateSpecificHouseOfLordsRace(stateCode)}>
                    View Race
                  </InternalLink>
                }
                people={groups.houseOfLords}
                title={<>House of Lords Race ({stateCode})</>}
              />
            </div>
          )}
          {!!stances.length && (
