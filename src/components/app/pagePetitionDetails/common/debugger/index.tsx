...
import { PetitionDataEditor } from './petitionDataEditor'
import { PetitionMilestonesEditor } from './petitionMilestonesEditor'

interface PetitionDebuggerProps {
  isOpen: boolean
  onClose: () => void
...
type DebuggerTab = 'data' | 'milestones'

export const PetitionDebugger = ({
  isOpen,
  onClose,
  petition,
  onPetitionChange,
...
}: PetitionDebuggerProps) => {
  const [activeTab, setActiveTab] = useState<DebuggerTab>('data')

  if (!isOpen) return null

  const tabs = [
    { id: 'data' as const, label: 'Petition Data' },
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="flex h-screen w-96 flex-col bg-white shadow-xl">
        <div className="flex items-center justify-between border-b p-4">
          <h2 className="text-lg font-semibold">Petition debugger</h2>
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          {activeTab === 'data' && (
            <PetitionDataEditor
              ...
              onMockRecentSignatoriesCountChange={onMockRecentSignatoriesCountChange}
              onMockSignaturesChange={onMockSignaturesChange}
              onPetitionChange={onPetitionChange}
              petition={petition}
            />
          )}
          {activeTab === 'milestones' && (
            <PetitionMilestonesEditor onPetitionChange={onPetitionChange} petition={petition} />
          )}
        </div>
      </div>
    </div>
